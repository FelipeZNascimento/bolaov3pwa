...
export const onLogin =
  ...
  async (dispatch: Dispatch<TFetchLogin>) => {
...
export const onRegister =
  (email: string, password: string, fullName: string, name: string) =>
  async (dispatch: Dispatch<TFetchRegister>) => {
    dispatch({ type: ACTIONTYPES.REGISTERING } as const);
    const cryptoPass = sha1(password).toString();
    const randomIcon =
      faIconsList[Math.floor(Math.random() * faIconsList.length)];
...
export const onUpdateUserPreferences =
  (icon: string, color: string) => async (dispatch: Dispatch<TUpdateUser>) => {
