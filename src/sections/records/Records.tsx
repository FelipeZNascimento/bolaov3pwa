...
const Records = () => {
  ...
  const { recordsParam, weekParam } = useParams<{
    recordsParam: string;
    weekParam: string;
  }>();
  ...
  useEffect(() => {
    if (filter.id !== null) {
      const { accumulated, orderBy, sortAsc, limit, season, week, userId } =
        filter;
      dispatch(
        fetchRecords(
          accumulated,
  ...
  useEffect(() => {
    const selectedButton = defaultButtons.find(
      (button) => button.route === recordsParam
    );
    if (
      selectedButton === undefined ||
      (selectedButton.needsLoggedUser && !loggedUser)
    ) {
      ...
      setFilter({
        ...selectedButton,
        week:
          weekParam !== undefined ? parseInt(weekParam) : selectedButton.week
      });
    }
