...
const BettableTeam = ({
  ...
  const winnerIconClass = classNames({
    ...
    [`color-gold ${styles.bright}`]:
      currentlySelectedByUser[extraType] === team.id
  });
  ...
  const wildCardIconClass = classNames({
    [`${styles.icon} ${styles.translucid}`]: !disabled,
    [styles.iconDisabled]: disabled,
    [`color-grey1 ${styles.bright}`]: selectedWildcards.find(
      (wc) => wc === team.id
    )
  });
  ...
  const isSelected =
    currentlySelectedByUser[extraType] === team.id ||
    selectedWildcards.find((wc) => wc === team.id);
  ...
  return (
    <div className={containerClass}>
      <Tooltip title="Vencedor" arrow>
        <Icon
          classes={{ root: `fas fa-crown ${winnerIconClass}` }}
          onClick={() => onClick(team, extraType)}
        />
      </Tooltip>
      ...
      {wildcardExtraType && (
        <Tooltip title="Wildcard" arrow>
          <Icon
            classes={{ root: `fas fa-running ${wildCardIconClass}` }}
            onClick={() => onClick(team, wildcardExtraType)}
          />
        </Tooltip>
      )}
    </div>
