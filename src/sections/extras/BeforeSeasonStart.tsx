...
const BeforeSeasonStart = ({
    ...
    const renderText = () => {
        return (
            <span>
                Para cada divisão, escolha 1 campeão <Icon classes={{ root: `fas fa-crown` }} /><br />
                Escolha também 3 wild cards <Icon classes={{ root: `fas fa-running` }} /> por conferência
            </span>
        )
    };
    ...
    const renderBetsStatus = () => {
        ...
        if (selectedExtraBets[EXTRA_BETS_VALUES.AFC_WILDCARD] !== null) {
            const wcBets = selectedExtraBets[EXTRA_BETS_VALUES.AFC_WILDCARD] as number[];
            numOfWCBets += wcBets.length;
        }
        if (selectedExtraBets[EXTRA_BETS_VALUES.NFC_WILDCARD] !== null) {
            const wcBets = selectedExtraBets[EXTRA_BETS_VALUES.NFC_WILDCARD] as number[];
            numOfWCBets += wcBets.length;
        }
    ...
    return (
        <>
            {(selectedExtraSection === EXTRA_SECTION.AFC || selectedExtraSection === EXTRA_SECTION.NFC)
