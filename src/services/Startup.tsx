...
const Startup = (props: any) => {
  ...
  const isOnResultsOrBets =
    currentPage === ROUTES.RESULTS.display ||
  ...
  useEffect(() => {
    if (!isLoadingUser && !loggedUser) {
      const currentPath = menuOptions.find((option) =>
  ...
  const fetchSectionMatches = () => {
    if (currentPage === ROUTES.RESULTS.display) {
      dispatch(
        fetchMatches(currentSeason as number, currentWeek as number) as any
      );
    } else if (currentPage === ROUTES.BETS.display) {
      dispatch(
        fetchUserBets(currentSeason as number, currentWeek as number) as any
      );
    }
  };
  ...
  const fetchBothRankings = () => {
    ...
    dispatch(
      fetchRanking(currentSeason as number, currentWeek as number) as any
    );
  };
  ...
  return (
    <>
      {isOnResultsOrBets && (
        <LinearProgress progress={progress} onUpdate={manualUpdate} />
      )}
      {props.children}
    </>
  );
